 ...
 * product list  --douzy
 */
define("banjiajia/web/1.0.0/scripts/order/userremiburse/list-debug", [ "$-debug", "pager-debug", "util-debug", "state-debug", "ace-elements-debug", "jggrid-debug", "gridlocale-debug", "validation-debug", "plugin-debug" ], function(require, exports, module) {
    ...
    var state = new State();
    state.init(_contentPath + "/system/config/get/getOrderRemiburseState.json");
    ...
    function teacherTr(data) {
        ...
        var user = data.user;
        var html = "";
        html += '<tr class="odd">' + '<td class=" ">' + userReimburse.reimburseNo + "</td>" + '<td class=" "><a href="' + _contentPath + "/order/detail/" + userReimburse.orderNo + '.html" target="_blank">' + userReimburse.orderNo + "</a></td>" + '<td class="">' + user.name + "</td>" + '<td class="">' + user.phone + "</td>" + '<td class=" ">' + util.fmoney(userReimburse.price / 100, 2) + "</td>" + '<td class=" ">' + state.get(state.OrderProcessState(), userReimburse.state) + "</td>" + '<td class=" ">' + (!!userReimburse.fininshTime ? userReimburse.fininshTime : "") + "</td>" + '<td class=" ">' + '<div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">' + (//'<a class="blue" href="'+_contentPath+'/order/userremiburse/show/'+userReimburse.id+'.html">'+
        //	'<i class="icon-zoom-in bigger-130"></i>'+
        //'</a>'+
        userReimburse.state != 2 ? '<a class="green" target="_blank" href="' + _contentPath + "/order/userremiburse/edit/" + userReimburse.reimburseNo + '.html">' + '<i class="icon-pencil bigger-130"></i>' + "</a>" : "") + //'<a class="red" href="#">'+
